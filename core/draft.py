...
def build(kind: str, case: dict) -> tuple[str, list[Block]]:
    ...
    if kind == "s138":
        ...
        # The raw answer ("Appointed as an authorised dealer in 2021...") was
        # printed verbatim as a numbered paragraph. The reference composes it
        # into the parties paragraph instead.
        rel = str(case.get("relationship") or "").strip()
        dirs = [d.get("name", "") for d in rows(case, "directors") if d.get("name")]
        if multi(case) and dirs:
            ...
            li.append(who + (f" {_as_sentence(rel, case)}" if rel else ""))
        else:
            li.append(rel or V(case, "relationship", "how the dealing arose"))
        li.append("That in the course of business and against orders placed by you, the Company sold, "
        ...
        def _phrase(stance: str) -> str:
            if stance.startswith("Admit"):
                return "admit only to the limited extent stated below and deny the remainder"
            if stance.startswith("Not aware"):
                return "are not aware of the averments and put your client to strict proof thereof"
            return "deny the contents thereof"

        # Consecutive paragraphs answered the same way, with nothing specific to
        # add, are grouped — a reply says "paras 1 to 7" once, not seven
        # identical lines. A paragraph whose stance has not been set is shown as
        # a blank to be completed, never silently denied.
        i = 0
        while i < len(_paras):
            p = _paras[i]
            ...
            if not stance:
                blocks.append(Block("p", text=(
                    f"With reference to para {p.get('n')} of the Said Notice, "
                    + blank("stance — admit / deny / not aware, and CEAT's answer"))))
                i += 1
                continue
            if body:
                blocks.append(Block("p", text=f"With reference to para {p.get('n')} of the Said Notice, "
                                              f"we {_phrase(stance)}. {body}"))
                i += 1
                continue
            j = i
            while (j + 1 < len(_paras)
                ...
                j += 1
            if j > i:
                ref = f"paras {_paras[i].get('n')} to {_paras[j].get('n')}"
            else:
                ref = f"para {p.get('n')}"
            blocks.append(Block("p", text=f"With reference to {ref} of the Said Notice, "
                                          f"we {_phrase(stance)}."))
            i = j + 1
