...
with st.sidebar:
    ...
    if st.button("Re-check models", **fit('button'), key="recheck"):
...
def ask(row):
    ...
    if q.kind in ("choice", "chips"):
        ...
        rows = clean_rows(CASE.get(q.table), names)
        import pandas as pd
        df = pd.DataFrame(rows if rows else [{c: "" for c in names}], columns=names, dtype=str)
        ...
        edited = st.data_editor(df, key=wk, num_rows="dynamic",
                                **fit('data_editor'), hide_index=True,
...
with left:
    ...
    if ls and ls[0] == KIND:
        ...
        if c2.button("Clear and start new", key=f"clr2_{KIND}", **fit('button')):
            st.session_state.case[KIND] = default_case(KIND)
            st.session_state.docs[KIND] = {}
            st.session_state.drafted[KIND] = False
            st.session_state.report.pop(KIND, None)
    ...
    go = st.button("Analyse everything and draft the notice"
                   ...
                   type="primary", **fit('button'), key=f"go_{KIND}")
    ...
    with tabs[4]:
        ...
        for i, e in enumerate(H):
            with st.container(border=True, key=f"glass_h_{i}"):
                ...
                c1, c2 = st.columns(2)
                if c1.button("Restore this case", key=f"rest_{i}", **fit('button')):
                ...
                c2.download_button("Download .txt",
                                   ...
                                   "text/plain", key=f"hdl_{i}", **fit('replace'))
